...
# STEP 14: I am doing it for all the Mediterranean samples, but with the replicates from POSADA concatenated
# I first moved POSADA concatenated files to the /store/usr7eco1/Fede_store/Marko/04_STACKS/cleaned directory
echo -n "\
T_AsBORD_1_S12	BORD
T_AsBORD_2_S14	BORD
T_AsBORD_3_S17	BORD
...
# STEP 15-A-iv: Run denovo_map on all the samples.
popmap=$top/info/popmap.tsv
echo "File containing the samples to do a test run is: $popmap"
...
for M in $M_values ;do
	...
	# After that use the R_only script to make the plots and infer which M and n values you want to use
	##########################################################################################################
done
