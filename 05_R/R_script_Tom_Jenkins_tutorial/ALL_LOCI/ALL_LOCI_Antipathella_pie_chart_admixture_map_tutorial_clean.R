# I first need to import my R environment file for the 'all loci' dataset
load("/home/markoterzin/Documents/Antipathella/R_script_validation/ALL_LOCI/Pop_genomics_ALL_LOCI.RData")
...
setwd("~/Documents/Antipathella/R_script_Tom_Jenkins_validation/ALL_LOCI")
...
# Explore data
data_final # This is the genind object I obtained already in the previous script
nLoc(data_final) # number of loci
nPop(data_final) # number of sites
nInd(data_final) # number of individuals
summary(data_final$pop) # sample size
...
subsites_data = sort(c("BORD","FAV","POS","PTF","SAV","Slucia"))
...
genind2structure(informative_data_final, file = "Antipathella_genotypes", pops = TRUE, markers = FALSE)

# Convert STRUCTURE file to .geno format
struct2geno("Antipathella_genotypes", ploidy = 2, FORMAT = 2, extra.column = 2)

# Run snmf algorithm

# My data
set.seed(123)
snmf1 = snmf("Antipathella_genotypes.geno",
             K = 1:9, # number of K ancestral populations to run
             repetitions = 10, # ten repetitions for each K
             entropy = TRUE, # calculate cross-entropy
             project = "new")

# Load snmf project
data_snmf1 = load.snmfProject("Antipathella_genotypes.snmfProject")
...
site.order_data = c("BORD","FAV","POS","PTF","SAV","Slucia")
qlong_data$Site_ord = factor(qlong_data$Site, levels = site.order_data)

# Adjust facet labels my Antipathella
levels(qlong_data$Site)
facet.labs_data = c("Bordighera","Favignana","Posada","Portofino","Savona","Santa Lucia")
levels(qlong_data$Site) = facet.labs_data
levels(qlong_data$Site)

# Define colour palette
pal_data = colorRampPalette(c("lightsalmon3","slategray4"))
...
admix.bar_data
ggsave("1.Antipathella_admixture_barplot.png", width=10, height=4, dpi=300)
...
pie_charts(avg_admix_data, site = "BORD", cols = cols_data)

# Apply function to all sites using for loop: Antipathella
pies_data = list()
...
coords_data = read.csv("coordinates_Ant.csv")
...
# Extract coordinates for each site: Antipathella
...
# Convert ggplot pie charts to annotation_custom layers: Antipathella
pies.ac_data = list()
...
# Add layers to basemap: Antipathella
pie.map_data = basemap_data + pies.ac_data
pie.map_data
ggsave("2.Antipathella.pie_charts_map.png", width = 8, height = 10, dpi = 300)

# Combine ggplots: Antipathella
...
save.image("~/Documents/Antipathella/R_script_Tom_Jenkins_validation/ALL_LOCI/LEA_all_loci.RData")
