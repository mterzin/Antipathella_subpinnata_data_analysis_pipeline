...
setwd("/home/markoterzin/Documents/Antipathella/Terzin_et_al_Antipathella_resubmission/Scripts/05_R/R_script_POP_GEN_stats")
...
C.genlight@gen # This is useful!!! It tells you which samples have a lot of missing data
C.genlight@n.loc # This tells me how many SNPs I have
C.genlight@ind.names # returns labels for each individual
C.genlight@loc.names # returns names for each locus/SNP
C.genlight@loc.all
C.genlight@chromosome
...
# performs 1000 permuatations (Monte Carlo test basically)
data_genepop.full
...
# But we want to remove false positives!!!
# B-H FDR Correction first, from this tutorial (using stats R package): 
...
# I adjust my p vals using 6 different methods
...
# Multiplying by 6 (for 6 populations)
data_genepop_loci.names <- data_genepop_loci.names[rep(
  row.names(data_genepop_loci.names), times=6), 1]
data_genepop_loci.names <- as.data.frame(data_genepop_loci.names)
# Combining the 2 dataframes to get loci names and BH corrected p vals 
P_loci.names_and_BY <- c(data_genepop_loci.names, data_BY_pvals)
P_loci.names_and_BY <- as.data.frame(P_loci.names_and_BY)

# Finding all loci where BH corrected p val is significant
...
###**** Missing data ****###
############################
pdf("03A_Missing_data_plot.pdf", 
width = 350, height = 6)
# Plot
info_table(data_genepop, type = "missing", plot = TRUE)
# Close the pdf file
dev.off()
...
###**** Ploidy ****###
############################
pdf("04_Ploidy_plot.pdf", 
width = 200, height = 10)
# Plot
data_genepop.ploidy <- info_table(data_genepop, 
                                          type = "ploidy", 
                                          plot = TRUE, 
                                          low = "black", 
                                          high = "orange")
...
data_summary_stats <- poppr(data_genepop)
...
pdf("05_MLGs_plot.pdf", 
width = 8, height = 6)
# Plot
data_genepop.tab <- mlg.table(data_genepop)
# Close the pdf file
dev.off()

# MLG analysis with a threshold set to 0.05
A_MLGs_thr_0.05 <- mlg.filter(data_final, threshold = 0.05, distance = "nei.dist", threads = 1L)
# Note: this is an arbitrary threshold!
...
data_no_low_coverage <- missingno(data_genepop, type = "geno", cutoff = 0.3)
...
#############################################################
# I want test the effect of 'Depth' and 'Population' on genetic structuring
...
nameStrata(data_final) <- ~DepthPopulation/Depth/Population
# let's look at the dataset now
data_final
# We see that we have three vectors of different names in the 'strata' slot for microbov (DepthPopulation, 
# Depth, Population)
...
########################################
# fsthet R package
# This is the package: https://cran.r-project.org/web/packages/fsthet/fsthet.pdf

###############################################################################################
# I have the same genepop file and it looks like theirs, I just need to see how to read it in #
###############################################################################################

# Preparing the genpop file first as in tutorial: I need a matrix with Population and Individual 
# information attached
data.outliers.1 <- as.matrix(data_final)
data.outliers.1
...
depthinfo <- strata$Depth
depthinfo

ind.names <- pop.data_final$Sample_ID
...
data.adonis.outliers.1 <- cbind.data.frame(popinfo, depthinfo, data.outliers.1)
...
# Now only keep ind.names and popinfo
data.outliers_first_2_cols <- data.outliers %>% select(popinfo, ind.names)


# The steps up until this point are important to keep only the
#        filtered individuals (with high coverage)             

gpop <- my.read.genepop("populations.snps.genepop")

# Now remove the pop column because it doesn't give me the locality names here
gpop$popinfo <- NULL

# Now remove the low coverage samples, to keep the 38 individuals as in data_final
gpop_new <- left_join(data.outliers_first_2_cols, gpop)

######################################
# Perfect, starting the stats now!!! #
######################################

# fsthet function is the most important one!!! It does pretty much everything to detect outliers

# Wright
out.dat.wright <- fsthet(gpop_new)
out.dat.wright

# Beta
out.dat.beta <- fsthet(gpop_new, fst.choice = "var")
out.dat.beta

# Betahat
out.dat.betahat <- fsthet(gpop_new, fst.choice = "betahat")
out.dat.betahat

# Theta
out.dat.theta <- fsthet(gpop_new, fst.choice = "theta")
out.dat.theta
...
fsts <- calc.actual.fst(gpop_new) # This is calculating Fst values using Wright model
fsts

fsts.theta <- calc.actual.fst(gpop_new,"theta") # Weir’s θ (theta) estimator, an alternative way to calculate Fst
fsts.theta

fsts.beta <- calc.actual.fst(gpop_new,"var") # A variance-based Fst, β (var). 
fsts.beta
# This approach calculates Fst as the variance in allele frequencies

fsts.betahat <- calc.actual.fst(gpop_new,"betahat") # Like beta, but size-corrected
fsts.betahat

# Generating quantiles
# Using boot.out
# The fst.boot function generates smoothed quantiles when you specify bootstrap=FALSE.
quant.out.wright<-fst.boot(gpop_new, fst.choice = "fst", bootstrap = FALSE)
quant.out.beta<-fst.boot(gpop_new, fst.choice = "var", bootstrap = FALSE)
quant.out.betahat<-fst.boot(gpop_new, fst.choice = "betahat", bootstrap = FALSE)
quant.out.theta<-fst.boot(gpop_new, fst.choice = "theta", bootstrap = FALSE)

## From the results of str(quant.out), you can see that fst.boot() returns a list data.frame with 
# three elements: 
# the bootstrapped values (Fsts), the bins used in the bootstrapping (Bins), and a list of 
# the upperand lower smoothed quantiles (V3)
str(quant.out.wright)
str(quant.out.beta)
str(quant.out.betahat)
str(quant.out.theta)

# This is just to get a summary
head(quant.out.wright[[3]][[1]])
head(quant.out.beta[[3]][[1]])
head(quant.out.betahat[[3]][[1]])
head(quant.out.theta[[3]][[1]])

# Get the quantiles
quant.list.wright<-ci.means(quant.out.wright[[3]])
quant.list.beta<-ci.means(quant.out.beta[[3]])
quant.list.betahat<-ci.means(quant.out.betahat[[3]])
quant.list.theta<-ci.means(quant.out.theta[[3]])

# Plotting
# Open pdf
pdf("06_Outliers_additional_plot.pdf", 
width = 5, height = 4)
# plot
# par(mar=c(4,4,1,1))
plot(fsts$Ht, fsts$Fst,pch=19,xlab="Ht",ylab="Fst",main="Wright")
points(quant.list.wright$LowHet,quant.list.wright$low,type="l",col="red")
points(quant.list.wright$UppHet,quant.list.wright$upp,type="l",col="red")

# par(mar=c(4,4,1,1))
plot(fsts.beta$Ht, fsts.beta$Fst,pch=19,xlab="Ht",ylab="Fst",main="Beta")
points(quant.list.beta$LowHet,quant.list.beta$low,type="l",col="red")
points(quant.list.beta$UppHet,quant.list.beta$upp,type="l",col="red")

# par(mar=c(4,4,1,1))
plot(fsts.betahat$Ht, fsts.betahat$Fst,pch=19,xlab="Ht",ylab="Fst",main="Betahat")
points(quant.list.betahat$LowHet,quant.list.betahat$low,type="l",col="red")
points(quant.list.betahat$UppHet,quant.list.betahat$upp,type="l",col="red")

# par(mar=c(4,4,1,1))
plot(fsts.theta$Ht, fsts.theta$Fst,pch=19,xlab="Ht",ylab="Fst", main="Theta")
points(quant.list.theta$LowHet,quant.list.theta$low,type="l",col="red")
points(quant.list.theta$UppHet,quant.list.theta$upp,type="l",col="red")
# Close pdf
dev.off()

##########################################################################################
# Export the outliers to see which loci they are, and how many there is depending on the #
# calculation used #
####################

write.table(out.dat.wright, 
file = "Spreadsheet_03_outliers_WRIGHT.tsv", 
sep = "\t", 
quote = F, 
row.names = F)

write.table(out.dat.beta, 
file = "Spreadsheet_03_outliers_BETA.tsv", 
sep = "\t", 
quote = F, 
row.names = F)

write.table(out.dat.betahat, 
file = "Spreadsheet_03_outliers_BETAHAT.tsv", 
sep = "\t", 
quote = F, 
row.names = F)

write.table(out.dat.theta, 
file = "Spreadsheet_03_outliers_THETA.tsv", 
sep = "\t", 
quote = F, 
row.names = F)

# Now try to extract all the values where it says TRUE to obtain the outliers
outliers_wb <- left_join(out.dat.wright, out.dat.beta, by = "Locus")
outliers_bt <- left_join(out.dat.betahat, out.dat.theta, by = "Locus")
outliers_ALL_calculations <- left_join(outliers_wb, outliers_bt, by = "Locus")

# Keep only the Loci and Outliers column
outliers_ALL_calculations <- select(outliers_ALL_calculations, 
                                    Locus, 
                                    Outlier.x.x, 
                                    Outlier.y.x, 
                                    Outlier.x.y, 
...
# Change column names to make it more intuitive
colnames(outliers_ALL_calculations)
# [1] "Locus" "Outlier.x.x" "Outlier.y.x" "Outlier.x.y" "Outlier.y.y"

outliers_ALL_calculations <- outliers_ALL_calculations %>% 
  rename(
Outlier.Wright = Outlier.x.x,
Outlier.Beta = Outlier.y.x,
Outlier.Betahat = Outlier.x.y,
Outlier.Theta = Outlier.y.y
  )

# Check
colnames(outliers_ALL_calculations)
# Yes, it changed
# [1] "Locus"   "Outlier.Wright"  "Outlier.Beta""Outlier.Betahat" "Outlier.Theta"

# Extract outlier loci when there is TRUE in any of the columns
total_number_of_outliers <- outliers_ALL_calculations %>% filter_all(any_vars(. %in% 'TRUE'))
# Now looking at those outliers detected in each of the 4 calculation methods
outliers_4_TRUE <- outliers_ALL_calculations %>% filter_at(vars(Outlier.Wright, Outlier.Beta, Outlier.Betahat, Outlier.Theta),
   all_vars(. %in% 'TRUE'))

# or in only select columns:
outliers.Wright <- outliers_ALL_calculations %>% filter_at(vars(Outlier.Wright), any_vars(. %in% 'TRUE'))
outliers.Beta <- outliers_ALL_calculations %>% filter_at(vars(Outlier.Beta), any_vars(. %in% 'TRUE'))
outliers.Betahat <- outliers_ALL_calculations %>% filter_at(vars(Outlier.Betahat), any_vars(. %in% 'TRUE'))
outliers.Theta <- outliers_ALL_calculations %>% filter_at(vars(Outlier.Theta), any_vars(. %in% 'TRUE'))

# Selecting outlier loci that were detected in 2 or more calculation methods
outliers_2_or_more_TRUE <- filter(outliers_ALL_calculations, 
                                  Outlier.Wright == "TRUE" & Outlier.Beta == "TRUE" |
                                  Outlier.Wright == "TRUE" & Outlier.Betahat == "TRUE" |
                                  Outlier.Wright == "TRUE" & Outlier.Theta == "TRUE" |
                                  Outlier.Beta == "TRUE" & Outlier.Betahat == "TRUE" |
                                  Outlier.Beta == "TRUE" & Outlier.Theta == "TRUE" |
                                  Outlier.Betahat == "TRUE" & Outlier.Theta == "TRUE")
# Double-check if this is a good solution, and if it takes into account all the cases!!!
# I think it does

outliers_3_or_more_TRUE <- filter(outliers_ALL_calculations, 
                                  Outlier.Wright == "TRUE" & Outlier.Beta == "TRUE" & Outlier.Betahat == "TRUE" |
                                    Outlier.Wright == "TRUE" & Outlier.Beta == "TRUE" & Outlier.Theta == "TRUE" |
                                    Outlier.Wright == "TRUE" & Outlier.Betahat == "TRUE" & Outlier.Theta == "TRUE" |
                                    Outlier.Beta == "TRUE" & Outlier.Betahat == "TRUE" & Outlier.Theta == "TRUE")
#                                    Outlier.Wright == "TRUE" & Outlier.Beta == "TRUE" & Outlier.Betahat == "TRUE" & Outlier.Theta == "TRUE")
# The last line was just to check if the number will change, and it didn't, so it should be good
  
# Now only selecting outlier loci in the genepop object
keeploc_2_or_more <- select(outliers_2_or_more_TRUE, # data
                    Locus) # column I want to select
keeploc_3_or_more <- select(outliers_3_or_more_TRUE, # data
                            Locus)
# Remove the X from loci name
keeploc_2_or_more <- gsub("[a-zA-Z]", "", as.character(keeploc_2_or_more$Locus))
keeploc_3_or_more <- gsub("[a-zA-Z]", "", as.character(keeploc_3_or_more$Locus))
# Filter loci in genind object
data_outliers_only_2_or_more <- data_final[loc = keeploc_2_or_more]
data_outliers_only_3_or_more <- data_final[loc = keeploc_3_or_more]

###################################
# Now make the data_final_neutral #
###################################
# Make a list of all loci names first
all.loci <- locNames(data_final)
# Now select only the neutral ones
neutral_2_or_more <- setdiff(all.loci, keeploc_2_or_more)
neutral_3_or_more <- setdiff(all.loci, keeploc_3_or_more)

# And keep only the neutral ones
data_final_neutral2 <- data_final[loc=neutral_2_or_more]
data_final_neutral3 <- data_final[loc=neutral_3_or_more]

# Check number of loci in genind obj
length(locNames(data_final))
length(locNames(data_outliers_only_2_or_more))
length(locNames(data_outliers_only_3_or_more))
  length(locNames(data_final_neutral2))
length(locNames(data_final_neutral3))

# It worked!!!
# Now I have 5 genind objects:
# All loci (data_final)
# Outliers only (data_outliers_only_2_or_more and data_outliers_only_3_or_more)
# Neutral loci only (data_final_neutral2 and data_final_neutral3)
...
# Open pdf
pdf("15_Single_discriminant_function.pdf", width = 9, height = 5)
# Plot
scatter(C.dapc,1,1, col=cols, bg="white",
scree.da=FALSE, legend=TRUE, solid=.4)
# Close the pdf file
dev.off()

# Now doing this using alleles and not PCAs
...
# Checking if inferred and actual clusters match 
pdf("15_actual_cluster_vs_inferred_no_NAs.pdf", 
width = 5, height = 5)
# Plot
table.value(table(C.dapc_alpha_optimal$assign, 
                  pop(data_final)), 
            col.lab=levels(pop(data_final)))
# Close the pdf file
dev.off()
...
# They are using a genclone object but since it can also be done with genlight objects I have decided 
# not to convert
...
#########################################
# So the results I got must be valid!!! #
#########################################
...
# But first I need to assign strata to my genlight object as well
strata(data_final) <- strata
nameStrata(data_final) <- ~DepthPopulation/Depth/Population
data_final
...
# Open pdf
pdf("16_AMOVA_BOTH_significance.pdf", width = 10, height = 6)
# Plot
plot(data_final_amova_signif)
# Close the pdf file
dev.off()

pdf("16_AMOVA_BOTH_significance_clone_correction.pdf", width = 10, height = 6)
# Plot
...
data_h <- genind2hierfstat(data_final_neutral2)
...
pdf('Fst_values_heatmap.pdf', width=10,height=6)
pheatmap(
  mat   = mat.obs,
  color = viridis(1000),
