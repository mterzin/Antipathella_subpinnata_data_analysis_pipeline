...
#This is done for all reads, and they will all be placed in one directory output directory [/store/usr7eco1/Fede_store/Marko/03_FastQC_quality_re-check]
...
#This is the output directory where I want to place all the files after the quality check
  ...
  # mkdir /store/usr7eco1/Fede_store/Marko/03_FastQC_quality_re-check

#Making the PREFIX variable to extract sample name
for DATASET in $(ls /store/usr7eco1/Fede_store/Marko/02_Quality_adapter_filtering_truncation/clean*.fastq); 
do
  PREFIX=$(basename $DATASET | cut -d "." -f 1,2)
...
#Then move the files to the output directory: /store/usr7eco1/Fede_store/Marko/01_FastQC
  mv /store/usr7eco1/Fede_store/Marko/02_Quality_adapter_filtering_truncation/*.html /store/usr7eco1/Fede_store/Marko/03_FastQC_quality_re-check
  mv /store/usr7eco1/Fede_store/Marko/02_Quality_adapter_filtering_truncation/*.zip /store/usr7eco1/Fede_store/Marko/03_FastQC_quality_re-check # be careful not to have other zipped files/filders in the directory!!! You only want to remove the output from this step

#Finally, remove the part in the name that is not needed: _R1_001

for file in /store/usr7eco1/Fede_store/Marko/03_FastQC_quality_re-check/*;
do
